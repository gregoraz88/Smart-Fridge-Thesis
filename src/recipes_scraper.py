...
def get_all_recipes_ar(page_num):
    ...
    try:
        ...
        print(get_recipe(base_url+r )for r in recipe_links)
...
def get_all_recipes_epi(page_num):
    ...
    try:
        ...
        print('Could not parse page  in get_all_recipes_epi{}'.format(url))
        return []


def scrape_recipe_box(scraper, site_str, page_iter, status_interval=50):
    print('entered scrape_recipe_box, this is the site', site_str)
    if args.append:
    ...
    if args.multi:
        ...
        for i in page_iter:
            ...
            for key_link, list in recipes.items():
                for key in list:
                    if key == 'title' or key == 'ingredients':
                        print('this is i: ' , i , ',', key + ':', list[key])
            if i % status_interval == 0:
                print('entering the saving')
...
def get_fn_letter_links():
    ...
    try:
        ...
        print('Could not parse page in get_fn_letter_links {}'.format(url))
...
def quick_save(site_str, recipes):
    print('quick saving', recipes,'at site ', site_str)
    save_recipes(
