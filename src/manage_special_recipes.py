...
#merging  special diet into one dictionary to 'final_special_diets' in special_diets folder
################################################################################
def merge_special_diets_dict():
    path = 'C:/Users/gregh/Desktop/thesis/Smart-Fridge-Thesis/special_diets'

    recipe_path = 'special_diets.json'
    recipe_path1 = 'special_diets_from_raw.json'

    with open(path + '/' + recipe_path) as json_file:
        special_recipes = json.load(json_file)
    with open(path + '/' + recipe_path1) as json_file:
        special_recipes1 = json.load(json_file)

    special_recipes.update(special_recipes1)
    save_data(path + '/'+ 'final_special_diets.json', special_recipes)

################################################################################
#merge_special_diets_dict()

#merging nutritional dictionary to 'final_nutritional_dict.json' in special_diets folder
################################################################################
def merge_nutritional_dict():
    path = 'C:/Users/gregh/Desktop/thesisSmart-Fridge-Thesis/special_diets'
...
def scrape_cuisine_recipes():
    ...
    path2 = 'Recipe-Analysis-master\\CuisineAnalyzer\\cuisinedata'
    ...
    for file in all_file:
        ...
        df = pd.read_csv(file)
        dict = df.to_dict()
        if name[0] != 'cuisine_data':
            all_cuisine_recipes[name[0]] = dict

    print(all_cuisine_recipes['american']['id'])
    print('------------------------------------------')
    print(all_cuisine_recipes['american']['ingredients'])
    save_data('C:/Users/gregh/Desktop/thesis/Smart-Fridge-Thesis/all_cuisine_data/final_cuisine_recipes.json',all_cuisine_recipes)
